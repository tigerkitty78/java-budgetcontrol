...
# 1. Load raw data
def load_raw_data():
    df = pd.read_csv('Navithma savings.csv')
...
class EnhancedFeatureEngineer:
    ...
    def transform(self, X):
        ...
        X['What is your personal income?'] = X['What is your personal income?'].replace(0, 1)  # Avoid division by zero
        X['What is your job?'] = X['What is your job?'].astype(str)  # Add this line
        # Basic features with fillna
        X['unwanted_spending'] = pd.Series(np.where(
    X['Do you live alone and cover your own expenses?'] == 1,
    pd.to_numeric(X['If you live alone, How much do you spend for unwanted things (entertainment/snacks/leisure ) monthly'], errors='coerce'),
    pd.to_numeric(X['If you live with family How much do you spend for unwanted things (entertainment/snacks/leisure ) monthly'], errors='coerce')
), index=X.index).fillna(0)

        
        X['basic_needs'] = pd.Series(np.where(
    X['Do you live alone and cover your own expenses?'] == 1,
    ...
    pd.to_numeric(X['If you live with family How much do you spend on basic needs monthly?'], errors='coerce')
), index=X.index).replace(0, 1).fillna(1)

        
        # Financial health markers with safeguards
        ...
        # pd.DataFrame(your_array).fillna(0)
        return X[self.feature_names].fillna(0)

# 3. Model Pipeline
def create_pipeline(model_type='xgboost'):
    preprocessor = ColumnTransformer([
        ('num', StandardScaler(), ['Age', 'What is your personal income?',
                                  'How much do you save monthly?', 'savings_ratio',
                                  'wants_ratio', 'emergency_fund_months',
                                  'debt_income_ratio', 'impulse_spend_ratio',
                                  'age_group']),
        ('cat', OneHotEncoder(handle_unknown='ignore'), ['What is your job?'])
    ])
...
class FinancialAdvisorSystem:
    ...
    def train(self, X_train, y_train):
        for target in y_train.columns:
            ...
            # Save trained model
            joblib.dump(self.models[target], f"{target}_3_model.joblib")
...
if __name__ == "__main__":
    try:
        ...
        # joblib.dump(label_encoders, 'label_encoders_INTERRUPTED.joblib')
